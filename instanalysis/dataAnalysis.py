...
def goForItBaby():
    ...
    # fixed bin size
    bins = np.arange(0, 24, 2) # fixed bin size

    plt.xlim([min(times)-5, max(times)+5])

    plt.hist(times, bins=bins, alpha=0.5)
    plt.title('Times (fixed bin size)')
    plt.xlabel('time (bin size = 2)')
    plt.ylabel('count')


    plt.savefig('./static/img/histogram.png',transparent=True,  bbox_inches='tight')
    ...
    return(best_hastag_counts) #dictionary with key as hastag and val as number of occurance
